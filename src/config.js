...
function mixChannel(base, target, percent) {
  return Math.round(base * (1 - percent) + target * percent);
}

function calculateShade(baseValue, tone) {
  const rgb = parseRGBString(baseValue);
  if (!rgb) return baseValue;

  const diff = tone - 500;
  const percent = Math.min(90, Math.abs(diff) / 5) / 100;
  const target = diff >= 0 ? 0 : 255;

  const [r, g, b] = rgb;
  const mixed = [mixChannel(r, target, percent), mixChannel(g, target, percent), mixChannel(b, target, percent)];
  return `rgb(${mixed.join(' ')})`;
}

function formatColorValue(_name, tone, value) {
  ...
  if (/^\d/.test(trimmed) && trimmed.includes(' ')) {
    return tone ? calculateShade(trimmed, tone) : `rgb(${trimmed})`;
  }
...
function createColorKeywordMap(prefix, cssProperty) {
  ...
  Object.entries(BASE_COLORS).forEach(([name, baseValue]) => {
    const isNumeric = /^\d/.test(`${baseValue}`.trim());

    // Base class without tone
    entries[`${prefix}-${name}`] = `${cssProperty}: ${formatColorValue(name, null, baseValue)}`;

    if (!isNumeric) {
      return;
    }

    SHADE_STOPS.forEach(tone => {
      entries[`${prefix}-${name}-${tone}`] = `${cssProperty}: ${formatColorValue(name, tone, baseValue)}`;
    });
  });
...
export function resolveColorValue(name, tone) {
  const baseValue = BASE_COLORS[name];
  if (!baseValue) return null;
  return formatColorValue(name, tone, baseValue);
}
